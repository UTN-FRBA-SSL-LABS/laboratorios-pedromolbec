...
> R: inicializa el carrito con 0 productos. Es importante llamarla para confirmar que un carrito no tiene nada dentro.
...
> R: línea 24, solo suma el precio del producto sin tener en cuenta la cantidad
...
> R: esta <= cuando debería ser <, ya que comienza en 0
...
> R: el test unitario testea funciones especificas, mientras que el de integracion las mezcla para testear que trabajen bien en conjunto. Test unitario detecto primero.
...
> R: porque en c el usuario tiene manejo completo de la memoria, a diferencia de Python/Java donde el programa se encarga. Entonces que un error en memoria es grave en c.
...
> R: que no tenes que estar adivinando donde esta el bug por minutos y minutos, sino que ves en consola cual funcion y en que salida fallo.
...
> R: no detecto el bug porque la cantidad de items era uno, entonces es lo mismo multiplicar por uno que directamente no multiplicar. Pese se suma cantidad, es necesario multiplicar por dicho numero, ya que de eso depende que de el valor correcto.
